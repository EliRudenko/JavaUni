...
//  чтобы в конце показывать общий итог по выполненным тестам
const testState = {
    total: 0,
    passed: 0,
    failed: 0,
...
function initApiTests() {
    //  кнопки для обычных API-тестов user/order/discount
...
function apiTestBtnClicked(e) {
    ...
    if(td) {
        ...
        // GET/POST отправляют заголовок
        //PUT/PATCH/DELETE не отправляют заголовок 403
...
function runApiErrorTest(endpoint, method, headers, resId, expectedOk, title) {
    // функция запуска теста с ошибкой
...
function registerTestResult(title, isOk) {
    ...
    //один запуск кнопки = один тест в статистике
    testState.total += 1;
    if (isOk) {
        testState.passed += 1;
    } else {
...
function updateTestSummary() {
    ...
    // общий результат, который нужен по ДЗ 1 2!!!!
